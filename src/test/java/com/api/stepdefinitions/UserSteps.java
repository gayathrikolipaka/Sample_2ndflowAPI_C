...
//    private final String apiKey = ConfigReader.getProperty("apiKey");
    ...
    // --- For Invalid JSON Format scenario ---
    ...
    public void i_have_an_invalid_json_payload_for_user_creation() {
        // Load invalid JSON from test data file (first entry)
        try {
            // The test data file should contain at least one invalid JSON string
            ...
            // For simplicity, let's assume the file contains an array of objects, each with an 'invalidJson' field
            // Example: [{..., "invalidJson": "{\"userName\": \"abc\", }"}]
            int idx = fileContent.indexOf("\"invalidJson\"");
            if (idx != -1) {
                int start = fileContent.indexOf(":", idx) + 1;
                int quote1 = fileContent.indexOf('"', start);
                int quote2 = fileContent.indexOf('"', quote1 + 1);
                invalidJsonPayload = fileContent.substring(quote1 + 1, quote2);
                // Unescape JSON string if needed
                invalidJsonPayload = invalidJsonPayload.replace("\\\"", "\"");
            } else {
                // Fallback: use a typical invalid JSON
                invalidJsonPayload = "{\"userName\": \"abc\", }";
            }
        } catch (IOException e) {
            // Fallback: use a typical invalid JSON
            invalidJsonPayload = "{\"userName\": \"abc\", }";
    ...
    public void i_send_post_request_to_create_user_invalid_json() {
    ...
    public void the_response_should_indicate_an_invalid_json_format_error() {
        // Typically, the error message is in the response body
        String responseBody = invalidJsonResponse.getBody().asString();
        // Check for common error indicators
        boolean hasInvalidJsonError = responseBody.toLowerCase().contains("invalid json") ||
        ...
        Assert.assertTrue(hasInvalidJsonError, "Expected invalid JSON format error in response, but got: " + responseBody);
    }
}